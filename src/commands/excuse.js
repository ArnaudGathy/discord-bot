...
function formatExcuses(body) {
    ...
    body.map((excuse, idx) => {
        message.addField(`Excuse #${idx + 1}`, `<@${excuse.author.id}>: ${excuse.content}`);
    });
    return message;
}
...
function logError(contextName, error, client, msg) {
    ...
    const embedMessage = new Discord.RichEmbed()
        ...
        .addField('Happened on Channel', `'${msg.channel.name}' <#${msg.channel.id}>`)
...
function getExcuseCmd(msg, client) {
    ...
    (async () => {
        try {
            ...
            msg.channel.send(`Il n'y a pas encore d'excuse. Utilise la commande: \`${excuseInfo}\``);
        } catch (error) {
            logError('Codexcuse', `Error during add excuse POST request: ${error} `, client, msg);
            msg.channel.send('Désolé, petit problème interne, j\'en ai notifié mes propriétaires');
        }
    })();
}
...
function addExcuse(msg, client, excuseContent) {
    ...
    if (msg.mentions.users != null) {
        ...
        if (msg.mentions.users.size === 0) {
            return msg.channel.send(`Il faut de la délation, tu dois mentionner l'auteur (avec une mention discord @pseudo) !`);
        }
    }
